...
# ----------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    order = Order.order(id: :desc).limit(1).first
    names = Item.joins(:order_items).where("order_id = #{order.id}").pluck(:name)
    # ------------------------------------------------------------
  ...
  it '18. returns the names of items for a users order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    third_order = Order.where("user_id = #{@user_3.id}").limit(3).last
    items_for_user_3_third_order = Item.joins(:order_items).where("order_id = #{third_order.id}").pluck(:name)
    # ------------------------------------------------------------
